...
learners_union = gunion(list(
  g_featless, g_enet, g_xgb, g_lgbm, g_ranger
))
...
Next we show the parameter space: 
```{r parameter_space_frek, echo = FALSE}


max_mtry <- as.integer(sqrt(ncol(data_F) - 2))

param_tbl <- tibble(
  Model                 = c("XGBoost",      "LightGBM",      "glmnet",     "Ranger"),
  `learning_rate / eta` = c("0.01 – 0.3",   "1e-3 – 0.2",    "",           ""),
  `leaves / depth`      = c("3 – 7 (depth)","16 – 32 (leaves)","",          ""),
  `iterations / trees`  = c("200 – 800",    "200 – 1000",    "",           "100 – 500"),
  subsample             = c("0.6 – 1",      "",              "",           ""),
  mtry                  = c("",             "",              "",           paste0("1 – ", max_mtry)),
  alpha                 = c("",             "",              "0 – 1",      ""),
  s                     = c("",             "",              "1e-4 – 1",   "")
)

gt(param_tbl) |>
  tab_header(title = md("**Parameter search space**"))
