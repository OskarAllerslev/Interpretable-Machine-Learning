...
```{r boxplot_kun,  fig.cap = "MSE boxplot", echo = FALSE}
...
if (!file.exists("~/Interpretable-Machine-Learning/aflevering/frekvens_model.rds" ) | kør_igen == "ja") {
  rr <- resample(
  task         = task_freq,
  learner      = at_inner,
  resampling   = outer_rsmp,
  ...
  saveRDS(rr, "~/Interpretable-Machine-Learning/aflevering/frekvens_model.rds")
} else {
  rr = readRDS("~/Interpretable-Machine-Learning/aflevering/frekvens_model.rds")
}



########## START: Tjekker Stratum fungerer i outer loop

# Loop through outer folds

# for (i in unique(outer_instance$fold)) {
#   train_ids <- outer_instance[fold == i, row_id]
#   
#   cat(sprintf("\nOuter Fold %d:\n", i))
#   cat("Train class distribution:\n")
#   print(prop.table(table(data_F[train_ids]$claim_indicator)))
# }
########## SLUT: Tjekker Stratum fungerer i outer loop 

##Hvordan tjekker vi inner loop distribution?
...
at_enet = auto(g_enet, "enet")
at_featureless = auto(g_featureless, "featureless")
at_rf = auto(g_rf, "featureless")


task = list(task_S)
learners = list(at_lgbm, at_xgb, at_enet, at_featureless, g_rf)
design = benchmark_grid(task = task, learners = learners, 
                        resampling = rsmp("cv", folds = 2))

kør_igen <- "Nej"

# if (!file.exists("~/Interpretable-Machine-Learning/aflevering/severity_model.rds" ) | kør_igen == "ja") {
#   rr_S <- resample(
#   task         = task_S,
#   learner      = inner_tuner_S,
#   resampling   = outer_cv,
#   store_models = TRUE
# )
#   saveRDS(rr_S, "~/Interpretable-Machine-Learning/aflevering/severity_model.rds")
# } else {
#   rr_S = readRDS("~/Interpretable-Machine-Learning/aflevering/severity_model.rds")
# }

if (!file.exists("~/Interpretable-Machine-Learning/aflevering/severity_model.rds" ) | kør_igen == "ja") {
  bmr_s = benchmark(design, store_models = TRUE)
  saveRDS(bmr_s, "~/Interpretable-Machine-Learning/aflevering/severity_model.rds")
} else {
  bmr_s = readRDS("~/Interpretable-Machine-Learning/aflevering/severity_model.rds")
}
