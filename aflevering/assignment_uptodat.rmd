...
<!-- if (!file.exists("auto_tuner_tweedie.rds")) { -->
<!--   at_twd$train(task_twd) -->
<!--   saveRDS(at_twd, "auto_tuner_tweedie.rds") -->
<!-- } else { -->
<!--   at_twd <- readRDS("auto_tuner_tweedie.rds") -->
<!-- } -->
...
# p1 <- g_featureless$plot()
# p2 <- g_lgbm$plot()
# p3 <- g_xgb$plot()
# p4 <- g_enet$plot()

# ved ikke lige hvordan man skal plotte det her
...
?? Skal vi have residualplot/histogram for vores bedste tweedie-model ovenfor?

____________________________________________________________________

```{r , fig.cap= "Residual plot, histogram for residuals, QQ-plot", echo = FALSE}

# twd_p1 <- mlr3verse::autoplot(at_lgbm$predict(task_twd), type = "residual")
# twd_p2 <- mlr3verse::autoplot(at_lgbm$predict(task_twd), type = "histogram")
# 
...
at_inner <- auto_tuner(
  ...
  resampling = rsmp("cv", folds = inner_folds, stratify = TRUE),
  measure = msr("classif.bbrier"),
  tuner = tnr("random_search"),
  term_evals = n_evals
)

outer_rsmp <- rsmp("cv", folds = outer_foldsm, stratify = TRUE)
