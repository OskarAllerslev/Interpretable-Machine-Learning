...
```{r, echo = FALSE, results=FALSE }
  data_trans <- function(data){
    ...
    agg <- data[, .(
      ...
      Distribution_channel = max(Distribution_channel),
...
```
```{r}
filtered_df <- subset(data, Cost_claims_year < 1 & N_claims_year != 0)

# View the number of such rows
nrow(filtered_df)
...
ggplot(data, aes(x = N_claims_year, y = Cost_claims_year)) +
  ...
  labs(title = "Scatter Plot of Cost of Claims vs Number of Claims (No Extremes)",
       x = "Number of Claims This Year",
       y = "Cost of Claims This Year") +
  theme_minimal()
```
...
```{r, out.width="50%" , fig.cap="Missing data plot, right axis shows numer of missing columns in that row, and the left axis show how many rows have this missingness pattern",echo = FALSE}
...
```{r corplot, fig.cap = "Correlation plot between the continious covariates", echo = FALSE}
